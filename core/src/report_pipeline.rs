...
pub fn build_detail_db_impl(
...

    {
        ...
        // Real per-directory inode owner captured in Phase 1 (dirowner_t*.bin),
        // ingested into `dir_owner_uid` (indexed by dir_id; -1 = unknown because
        // the dir was never stat'd successfully). Fall back to 0 for unknown so
        // detail_dirs.owner_uid carries a real owner where we have one and does
