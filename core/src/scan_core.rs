...
/// creating its own, so the consumer can poll them for real-time progress.
/// The fields hold `Arc<Atomic*>` so cloning the struct is cheap and shares
...
impl ScanProgress {
    ...
    /// Current (files, dirs, size) snapshot — a cheap relaxed read.
...
pub fn run_scan_core(
    ...
    // When an external ScanProgress sink is supplied, share its atomics so the
    // caller (e.g. the CLI TUI thread) sees live counts; otherwise allocate our
    // own. `has_external_progress` also suppresses the stdout progress line so
    // it doesn't fight with a TUI rendering to the same terminal.
    let has_external_progress = progress.is_some();
    let sink = progress.unwrap_or_default();
    let prog_files = sink.files.clone();
    ...
    while !done.load(Ordering::SeqCst) {
        // py.check_signals() — no-op without pyo3
        ...
        if elapsed_secs >= 10 {
            ...
            // When an external progress sink drives a TUI, suppress this stdout
    ...
    let mut uid_sizes_map = serde_json::Map::new();
    for (uid, size) in &g.uid_sizes {
        uid_sizes_map.insert(uid.to_string(), serde_json::json!(size));
    }
    let mut uid_files_map = serde_json::Map::new();
    for (uid, files) in &g.uid_files {
        uid_files_map.insert(uid.to_string(), serde_json::json!(files));
    }
    let dir_owner_list: Vec<u32> = g.dir_owner_uids.iter().copied().collect();

    let mut result_map = serde_json::Map::new();
    result_map.insert("total_files".into(), serde_json::json!(g.total_files));
    result_map.insert("total_dirs".into(), serde_json::json!(g.total_dirs));
    result_map.insert("total_inodes".into(), serde_json::json!(g.total_inodes));
    result_map.insert("total_size".into(), serde_json::json!(g.total_size));
    result_map.insert("detail_tmpdir".into(), serde_json::json!(&tmpdir_str));
    result_map.insert("dir_tmpdir".into(), serde_json::json!(&tmpdir_str));
    result_map.insert("uid_sizes".into(), serde_json::Value::Object(uid_sizes_map));
    result_map.insert("uid_files".into(), serde_json::Value::Object(uid_files_map));
    result_map.insert("dir_owner_uids".into(), serde_json::json!(dir_owner_list));
    result_map.insert("permission_issues_count".into(), serde_json::json!(g.permission_issues_count));
    result_map.insert("engine".into(), serde_json::json!(engine));
    if engine == "production" {
        result_map.insert("schema".into(), serde_json::json!("check-disk-scan"));
    }
